...
### ✅ 阶段 1: 基础架构搭建（已完成）
- 配置项目依赖（libs.versions.toml、build.gradle.kts）
- 创建基础模块（bili_sdk、common_ui_core、feature_annotations）
- 初始化应用核心（BTubeApp、Koin依赖注入）
- 建立核心工具类（Constants、HttpClientFactory、EventBus等）

### ✅ 阶段 2: SDK 模块开发（已完成）
- 数据模型层（model_v2/）
- WBI签名系统（wbi/）
- API接口层（AuthApi, UserApi）
