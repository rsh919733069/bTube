package com.example.btube.features.main.home.recommend
...
fun RecommendScreen(
    ...
    PullToRefreshBox(
        ...
        LazyVerticalGrid(
            ...
            items(
                count = recommends.itemCount,
                key = { recommends.peek(it)?.bvid ?: it }
            ) { index ->
                val item = recommends[index]
                if (item != null) {
                    VerticalVideoCard(
                        pic = item.pic.ifEmpty { item.pic43 },
                        title = item.title,
                        ownerName = item.owner?.name ?: "",
                        viewCount = item.stat?.view?.let { formatCount(it) } ?: "0",
            ...
            if (recommends.loadState.append is LoadState.Loading) {
